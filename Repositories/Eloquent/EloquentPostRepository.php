...
class EloquentPostRepository extends EloquentBaseRepository implements PostRepository
{
    ...
    public function allTranslatedIn($lang)
    {
        return $this->model->whereHas('translations', function (Builder $q) use ($lang) {
            $q->where('locale', "$lang");
            $q->where('title', '!=', '');
        })->with('translations','user')->whereStatus(Status::PUBLISHED)->orderBy('created_at', 'DESC')->get();
    }
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function allTranslatedInAndCurrentPublish($lang)
    {
        return $this->model->whereHas('translations', function (Builder $q) use ($lang) {
            $q->where('locale', "$lang");
            $q->where('title', '!=', '');
    ...
    public function getPostForPage($currentPage, $postsPerPage){
        return $this->model
        ->whereStatus(Status::PUBLISHED)
        ->whereDate('publish_date', '<=', Carbon::today()->toDateString())
        ->orderBy('created_at', 'DESC')
